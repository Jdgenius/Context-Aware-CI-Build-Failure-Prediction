...
def run_random_forest_inference(
    ...
    *,
    ...
    resolved_test_fraction = (
        test_fraction
        if test_fraction is not None
        else training_config.get("test_fraction", 0.2)
    )
    resolved_seed = seed if seed is not None else training_config.get("seed", 0)
    resolved_shuffle_splits = (
        shuffle_splits
        if shuffle_splits is not None
        else training_config.get("shuffle_splits", True)
    )
    ...
    splits = load_sample_splits(
        ...
        seed=resolved_seed,
        shuffle=resolved_shuffle_splits,
    ...
    if splits.test.labels.shape[0] == 0:
        raise ValueError("No samples were assigned to the test split.")

    features = splits.test.features.numpy()
    labels = splits.test.labels.to(torch.int64).numpy()
    model = checkpoint["model"]
    predictions = model.predict(features).astype(int).tolist()
    test_labels = labels.astype(int).tolist()
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = run_random_forest_inference(
        ...
        seed=args.seed,
        shuffle_splits=False if args.no_shuffle_splits else None,
